...
def get_embedding_function(api_key):
    """
    Return an GoogleGenerativeAIEmbeddings object, which is used to create vector embeddings from text.
    The embeddings model used is "text-embedding-ada-002" and the Gemini API key is provided
    as an argument to the function.

    Parameters:
        api_key (str): The Gemini API key to use when calling the GoogleGenerativeAIEmbeddings API.

    Returns:
        GoogleGenerativeAIEmbeddings: An GoogleGenerativeAIEmbeddings object, which can be used to create vector embeddings from text.
    """
...
def create_vectorstore_from_texts(documents, api_key, file_name):
    ...
    :param api_key: The Gemini API key used to create the vector store
    :param file_name: The name of the file to associate with the vector store
...
def query_document(vectorstore, query, api_key):
    ...
    :param api_key: The Gemini API key to use when calling the Gemini Embeddings API
